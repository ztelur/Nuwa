...
//                    def oldNuwaDir = new File("${project.buildDir}/outputs/nuwa")
//                    if (oldNuwaDir) { //可能为空
...
//                    }
                    ...
                    // 在clean之前保持文件
                    def oldNuwaDir
                    Closure nuwaReadHashBeforeClean = {
                        oldNuwaDir = new File("${project.buildDir}/outputs/nuwa")
                        if (oldNuwaDir) { //可能为空
                            def mappingFile = NuwaFileUtils.getVariantFile(oldNuwaDir, variant, MAPPING_TXT)
                            NuwaAndroidUtils.applymapping(proguardTask, mappingFile)
                        }
                    ...
                    def readBeforeClean = "nuwa${variant.name.capitalize()}ReadBeforeClean"
                    project.task(readBeforeClean) << {
//                        if (patchDir) {
//                            NuwaAndroidUtils.dex(project, patchDir)
//                        }
                    }
                    def nuwaReadBeforeCleanTask = project.tasks[readBeforeClean]
                    nuwaReadBeforeCleanTask.doFirst(nuwaReadHashBeforeClean)
                    cleanTask.dependsOn nuwaReadBeforeCleanTask
                    //////////////////////////////////////////////////////////
                    ...
                    Closure nuwaPrepareClosure = {
                        ...
                        println "not contain " + applicationName
